...
def downloadSpecificRecords(request):
    ...
    for poll in polls:
        all_responses += list(get_object_or_404(Question,pk=poll).response_set.filter(active=1,timestamp__gt=datetime.datetime(2018,5,15,16),timestamp__lt=datetime.datetime(2018,5,15,18)))
