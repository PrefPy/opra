...
class Matcher:
	...
	#studentFeatures is a dict from student to feature vector
	#e.g "student1" -> (0, 1, 3.8)
	#the feature vector gives a score for each feature for this student
	...
	def __init__(self, studentPrefs, studentFeatures, classCaps, classFeatures):
		...
		self.studentFeatures = studentFeatures
		...
		#returns a complete ranking over students for one specific class
		#classFeature is the feature vector for a class

		def makeClassPref(classFeature):

			#return list of students, sorted by dot product, then student name (for tiebreaking)
			return sorted(studentPrefs.keys(), key=lambda x: (dot(classFeature, studentFeatures[x]), x), reverse=True)

		#call makeClassPref on each classFeature vector to make the preferences
		self.classPrefs = {c: makeClassPref(f) for c, f in classFeatures.items()}
		'''
	...
	def match(self):
		...
		f = True;
		studentMatched = {}
		for oneStudent in self.studentFeatures:
			studentMatched[oneStudent] = False;

		classMatching = {}
		for oneClass in self.classFeatures:
			classMatching[oneClass] = []

		while f:
			for oneStudent in studentMatched:
				if studentMatched[oneStudent]:
					continue
				if self.studentPrefs[oneStudent] == []:
					continue
				oneClass = self.studentPrefs[oneStudent].pop(0)
				classMatching[oneClass].append(oneStudent)
				if len(classMatching[oneClass]) > self.classCaps[oneClass]:
					classMatching[oneClass].sort(key=lambda x: dot(self.classFeatures[oneClass], self.studentFeatures[x][oneClass]), reverse=True)
					popStudent = classMatching[oneClass].pop(-1)
					studentMatched[popStudent] = True


			# Check if all the classes are full.
			f = False;
			for oneClass in classMatching:
				if len(classMatching[oneClass]) < self.classCaps[oneClass]:
					f = True
					break
		for oneClass in classMatching:
		...
		'''
		self.studentMatching = studentMatched

		#populate classMatching from studentMatching
		for s, c in studentMatched.items():
			self.classMatching[c].append(s)
		'''
		self.classMatching = classMatching
